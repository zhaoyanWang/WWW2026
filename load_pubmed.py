...
def load_pubmed(args, per_classnum=10, seed=0, thred=1.0, lam=-1, model_type=''):
    data = get_pubmed_casestudy(False, seed)
    np.random.seed(seed)

    # Randomly drop nodes (optional, use args to control)
    ...
    # Split data into training, validation, and testing sets using ratio-based approach
    ...
    # Load embeddings
    new_embed = np.load(dir + '/pubmed_process/new_nodes_optimized_supcon_rag_pubmed.npy')
    text_embed = np.load(dir + '/pubmed_process/original_nodes_optimized_supcon_rag_pubmed.npy')
    ...
    sim = torch.mm(torch.from_numpy(new_embed), torch.from_numpy(text_embed[:N]).t())
    ...
    if model_type == 'Edge':
        ...
        # For edge prediction, we can create additional prediction edges based on new embeddings
        predict_sim = torch.mm(torch.from_numpy(new_embed), torch.from_numpy(text_embed).t())
        predict_new_edge = torch.nonzero(predict_sim > thred)
        predict_new_edge[:,0] = predict_new_edge[:,0] + N
        data.predict_edge_index = predict_new_edge.transpose(0,1)
