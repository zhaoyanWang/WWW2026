...
class GAT_Encoder(torch.nn.Module):                           
    def __init__(self, in_channels, hidden_dim, out_channels, heads=8):
        super(GAT_Encoder, self).__init__()
        self.conv1 = GATConv(in_channels, hidden_dim // heads, heads=heads)
...
def node_classification():
    ...
    results_file = "results/all_results_label_allchange_rag_pubmedGAT.csv"
            ...
            for feature_drop_ratio in feature_drop_ratios:
                for train_ratio in train_ratios:
                    ...
                    for s in [42, 48, 13, 25, 68]:
                        ...
                        Node = GAT_Encoder(num_features, hidden_dim, out_channels, heads=8).to(device)
                    ...
                    pd.DataFrame([experiment_result]).to_csv("results/all_results_label_allchange_rag_pubmedGAT.csv", mode='a', header=False, index=False)
                    print(experiment_result)
    print("Results saved to results/all_results_label_allchange_rag_pubmedGAT.csv")
